    ...
    <section id="skills" className="mb-12 scroll-mt-16 lg:mb-20 lg:scroll-mt-24">
      ...
      <ul className="flex flex-wrap gap-3">
        {portfolioData.skills.map((skill, idx) => (
          ...
          >
            {skill.icon && !(skill as any).darkIcon && (
              <img src={skill.icon} alt={skill.name} className="w-4 h-4 object-contain" />
            )}
            {skill.icon && (skill as any).darkIcon && (
              <>
                <img src={skill.icon} alt={skill.name} className="w-4 h-4 object-contain dark:hidden" />
                <img src={(skill as any).darkIcon} alt={skill.name} className="w-4 h-4 object-contain hidden dark:block" />
              </>
            )}
            {skill.name}
          </li>
